...
class KDTree
{
  ...
  private:
     ...
     */
    void build_KDTree(vector<Point<Dim>>&points, KDTreeNode*& subroot, long left, long right, int d);
    void partition(vector<Point<Dim>>& points, size_t left, size_t right, int curr_dim, size_t partition_idx);
    void swap(Point<Dim>& point1, Point<Dim>& point2); 

    const Point<Dim>& findNearestNeighbor(KDTreeNode* subroot, 
                                          const Point<Dim>& target, 
                                          int d, Point<Dim>& closest) const;

    void _destroy(KDTreeNode*& subroot);

    void _copy(KDTreeNode*& this_tree_subroot, const KDTreeNode*& other_tree_subroot);
};
